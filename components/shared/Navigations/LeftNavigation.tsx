        ...
        {Data.LeftNavigation.NavItems.map((item: any, index: number) => {
          return (
            <Link
              key={index}
              href={item.Route}
              className={`w-full h-[48px] text-white-main flex justify-start items-center px-8 font-OpenSans text-[18px] leading-[28px] tracking-[-0.01em] hover:bg-brand-tertiary ${
                Router.pathname === item.Route
                  ? "font-semibold bg-brand-tertiary "
                  : "font-normal"
              } `}
            >
              {item.Name}
