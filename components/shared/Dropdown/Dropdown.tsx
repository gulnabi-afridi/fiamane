...
function Dropdown({
  ...
  return (
    <div ref={domNode} className={`relative Dropdown ${Styles}`}>
      ...
      >
        <p
          className={`font-OpenSans font-normal text-[14px] leading-[28px] outline-none ${
            Placeholder === ValueTitle ? "text-[#B3B3B3]" : "text-[#1A1A1A]"
          }`}
        >
          {ValueTitle}
        </p>
        <Icons.ArrowDown
          ...
          stroke={Placeholder === ValueTitle ? "#B3B3B3" : "#1A1A1A"}
          fill="none"
        />
      </div>
      {State && (
